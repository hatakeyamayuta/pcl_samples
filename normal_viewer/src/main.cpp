...
void remove_normal(pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &norm_cloud){
	...
	outfiles.open("normals_hist.txt",std::ios::out);
...
int main (int argc, char* argv[]){
	...
	std::cout <<normal->points[0].normal_x << std::endl;
	...
	std::cout <<norm_cloud->points[0] << std::endl;
	std::cout <<norm_cloud->points[0] << std::endl;
	...
	viewer.addArrow(pcl::PointXYZ(x*a, y*b, z*c),cloud->points[20000],0.0, 0.0, 1.0,false);
